...
results_dir = "./results_NIRPed/dt_results_%s_B%d_%s" % (subset, max_boxes, str(score_threshold_cls)[2:])
if not os.path.exists(results_dir):  # if it exist already
    # shutil.rmtree(results_dir)     # reset the results directory 重置结果目录,也就是删除现有的文件夹images-optional
    os.makedirs(results_dir)  # 在程序当前目录下创建一个新的文件夹名为results_files_path = "results"

time_cost_file = os.path.join(results_dir, 'time_cost_list_{}.json'.format(subset))
...
bounding_boxes_file = os.path.join(results_dir, 'DtResults_{}.json'.format(subset))
...
def predict(test_images_json):  # 预测args_指定的图片
    ...
    for index_image in range(num_imgs_all):
        ...
        # image_path = os.path.join('E:\\Datasets\\NIRPed2021\\NIRPed\\images\\{}\\{}'.format(subset, img_name))
        # image_path = '.\\data\\miniNIRPed\\images\\{}\\{}'.format(subset, img_name)
        image_path = os.path.join(cfg.img_dir, img_name)


        if image_path == None:
            print('Notion:{} do not exist.'.format(image_path))
            continue
        else:
            predict_single_image(image_path)

        if index_image % 50 == 0:
            print('N={}/{}:{} exist.'.format(index_image + 1, num_imgs_all, image_path))  # 打印预测图像的名称

            bounding_boxes.sort(key=lambda x: float(x['confidence']), reverse=True)  # sort detection-result by decreasing confidence

            outfile = open(os.path.join(results_dir,  'DtResults_%s.json' %(subset)), 'w')
            json.dump(bounding_boxes, outfile)
            outfile.close()
            outfile = open(os.path.join(results_dir, 'time_cost_list_%s.json' %(subset)), 'w')
            json.dump(time_cost_list, outfile)
            outfile.close()

    bounding_boxes.sort(key=lambda x: float(x['confidence']), reverse=True)  # sort detection-result by decreasing confidence

    outfile = open(os.path.join(results_dir, 'DtResults_{}.json'.format(subset)), 'w')
    json.dump(bounding_boxes, outfile)
    outfile.close()

    mean_time_cost = np.round(np.mean(time_cost_list[1:], axis=0), 6)
    time_cost_list[0] = mean_time_cost.tolist()
    outfile = open(os.path.join(results_dir, 'time_cost_list_{}.json'.format(subset)), 'w')
    json.dump(time_cost_list, outfile)
    outfile.close()

if __name__ == '__main__':
    if subset == 'val':
        print('Detecte subset {}'.format(cfg.val_anno))
        cfg.img_dir = cfg.val_img_dir
        predict(cfg.val_anno)
    elif subset == 'test':
        print('Detecte subset {}'.format(cfg.test_anno))
        cfg.img_dir = cfg.test_img_dir
        predict(cfg.test_anno)